...
DEFAULT_USER_AGENT: Final[str] = "ColaFetch/0.1 (+cn-download)"
...
class CninfoDiscoveryClient:
    ...
    def download_report_pdf(self, candidate: CnReportCandidate) -> DownloadedReportAsset:
        ...
        tmp_dir = Path(tempfile.gettempdir()) / "cola_fetch_cn_downloads"
...
# 注意：同 fiscal_year 内按此 key 升序，Q 值越大代表季度越早，
# 故 Q4 值最小（最新季度排前），保证"最新季报"取到最新季度而非最旧。
# 须与 hkexnews_downloader.py 的 _PERIOD_SORT_KEY 保持一致。
