...
_UNCONFIGURED_USER_AGENT = "ColaFetch admin@example.com"
...
class SecDownloader:
    ...
    def __init__(
        ...
        self._throttle_triggered: bool = False  # 本次实例是否触发过 SEC 限流（429/503）
    ...
    def was_throttled(self) -> bool:
        """返回本次实例是否触发过 SEC 限流（429/503）。

        供 CLI 层在下载失败时检测：若触发过限流，应提示用户设置真实 UA。

        Args:
            无。

        Returns:
            触发过限流返回 ``True``；否则 ``False``。

        Raises:
            无。
        """

        return self._throttle_triggered

    def normalize_ticker(self, ticker: str) -> str:
        """标准化 ticker。

        代理到 ``download_financial_report.fins.ticker_normalization`` 真源；识别失败时回退到
        ...
        while attempt_index < self._max_retries:
            ...
            try:
                ...
                if response.status_code in _THROTTLE_STATUS_CODES and throttle_retries_remaining > 0:
                    ...
                    self._throttle_triggered = True  # 标记本次实例触发过限流
    ...
    def _resolve_user_agent(self, configured_user_agent: Optional[str]) -> str:
        ...
        优先级：显式传入 > 环境变量 SEC_USER_AGENT > 预设 fallback。

        预设 UA ``ColaFetch admin@example.com`` 用 RFC 2606 保留的
        ``example.com`` 域名作占位邮箱，单次或低频访问够用；高频访问触发
        SEC 限流时由 CLI 层提示用户设置真实 UA（见 ``was_throttled``）。
